...
class Cluster:
    def __init__(self, data_list = np.array([]), cluster_list = np.array([]), dsim = 0, medoid = -1, e_g = 0):
...
class HC_ward_medoid:
    def __init__(self, input_data):
        ...
        self.medoid_list = np.full((self.data_list.shape[0], self.data_list.shape[0]), -1)
        self.e_g_list = np.full((self.data_list.shape[0], self.data_list.shape[0]), -1.)


    def fit(self):
        #初期クラスタリスト作成
        for i in range(self.data_list.shape[0]):
            cluster_obj = Cluster(data_list = np.array([i]), medoid = i)
    ...
    def dsim_list_set(self):
        for i in range(self.data_list.shape[0] - 1):
            ...
            self.e_g_list[i + 1:, i] = self.data_list[i + 1:, i]
            self.medoid_list[i + 1:, i] = i


    def cal_dsim(self, cluster_1_index, cluster_2_index):
        new_dsim_list = np.array([])
        new_e_g_list = np.array([])
        new_medoid_list = np.array([], dtype = int)
        cluster_1_2_data = np.append(self.cluster_list[cluster_1_index].get_data_list(), self.cluster_list[cluster_2_index].get_data_list())
        e_g_1_2 = self.e_g_list[cluster_1_index, cluster_2_index]
        ...
        for cluster_index in range(self.cluster_list.shape[0]):
            ...
            dsim_sum_list = np.array([])
            for i in np.append(self.cluster_list[cluster_index].get_data_list(), cluster_1_2_data):
                dsim_sum = 0.
                for j in np.append(self.cluster_list[cluster_index].get_data_list(), cluster_1_2_data):
                    dsim_sum += self.data_list[i, j]

                dsim_sum_list = np.append(dsim_sum_list, dsim_sum)

            new_e_g = dsim_sum_list.min()
            new_medoid_index = np.where(dsim_sum_list == new_e_g)[0][0]

            new_dsim_list = np.append(new_dsim_list, new_e_g - self.cluster_list[cluster_index].get_e_g() - e_g_1_2)
            new_e_g_list = np.append(new_e_g_list, new_e_g)
            new_medoid_list = np.append(new_medoid_list, new_medoid_index)

        return new_dsim_list, new_medoid_list, new_e_g_list
    ...
    def updata(self):
        ...
        new_dsim_list, new_medoid_list, new_e_g_list = self.cal_dsim(cluster_1_index = min_dsim_search_list_index[0][0], cluster_2_index = min_dsim_search_list_index[1][0]) #追加する非類似度行列を取得
        new_cluster = Cluster(data_list = np.append(cluster_1.get_data_list(), cluster_2.get_data_list()), cluster_list = np.array([cluster_1, cluster_2]), dsim = min_dsim, medoid = self.medoid_list[min_dsim_search_list_index[0][0], min_dsim_search_list_index[1][0]], e_g = self.e_g_list[min_dsim_search_list_index[0][0], min_dsim_search_list_index[1][0]]) #クラスタ作成
        ...
        self.medoid_list = np.delete(self.medoid_list, min_dsim_search_list_index[0][0], 0)
        self.medoid_list = np.delete(self.medoid_list, min_dsim_search_list_index[0][0], 1)
        self.medoid_list = np.delete(self.medoid_list, min_dsim_search_list_index[1][0], 0)
        self.medoid_list = np.delete(self.medoid_list, min_dsim_search_list_index[1][0], 1)
        self.e_g_list = np.delete(self.e_g_list, min_dsim_search_list_index[0][0], 0)
        self.e_g_list = np.delete(self.e_g_list, min_dsim_search_list_index[0][0], 1)
        self.e_g_list = np.delete(self.e_g_list, min_dsim_search_list_index[1][0], 0)
        self.e_g_list = np.delete(self.e_g_list, min_dsim_search_list_index[1][0], 1)
        ...
        self.medoid_list = np.append(self.medoid_list, np.array([new_medoid_list]), 0)
        self.medoid_list = np.append(self.medoid_list, np.full((self.medoid_list.shape[0], 1), -1), 1)
        self.e_g_list = np.append(self.e_g_list, np.array([new_e_g_list]), 0)
        self.e_g_list = np.append(self.e_g_list, np.full((self.e_g_list.shape[0], 1), -1), 1)
    ...
    def create_dendrogram(self):
        ...
        plt.savefig("HC_ward_medoid.png")
        plt.show()
