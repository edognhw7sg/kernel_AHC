...
class Cluster:
    def __init__(self, data_list = np.array([]), cluster_list = np.array([]), dsim = 0, medoid = -1):
...
class HC_centroid_medoid:
    ...
    def cal_dsim(self, cluster_1_index, cluster_2_index):
        ...
        cluster_1_data = self.cluster_list[cluster_1_index].get_data_list()
        cluster_2_data = self.cluster_list[cluster_2_index].get_data_list()
        dsim_sum_list = np.array([])
        for i in np.append(cluster_1_data, cluster_2_data):
            dsim_sum = 0.
            for j in np.append(cluster_1_data, cluster_2_data):
                dsim_sum += self.data_list[i, j]

            dsim_sum_list = np.append(dsim_sum_list, dsim_sum)

        new_medoid_index = np.where(dsim_sum_list == dsim_sum_list.min())[0][0]

        for cluster_index in range(self.cluster_list.shape[0]):
            if cluster_index == cluster_1_index or cluster_index == cluster_2_index:
                continue

            new_dsim_list = np.append(new_dsim_list, self.data_list[self.cluster_list[cluster_index].get_medoid(), new_medoid_index])

        return new_dsim_list, new_medoid_index
    ...
    def updata(self):
        ...
        new_dsim_list, new_medoid_index = self.cal_dsim(cluster_1_index = min_dsim_list_index[0][0], cluster_2_index = min_dsim_list_index[1][0]) #追加する非類似度行列を取得
        new_cluster = Cluster(data_list = np.append(cluster_1.get_data_list(), cluster_2.get_data_list()), cluster_list = np.array([cluster_1, cluster_2]), dsim = min_dsim, medoid = new_medoid_index) #クラスタ作成
    ...
    def create_dendrogram(self):
        ...
        plt.savefig("HC_centroid_medoid.png")
        plt.show()
