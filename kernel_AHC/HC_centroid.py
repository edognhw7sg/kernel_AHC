...
class HC_centroid:
    ...
    def cal_dsim(self, cluster_1_index, cluster_2_index):
        ...
        for cluster_index in range(self.cluster_list.shape[0]):
            ...
            cluster_num = self.cluster_list[cluster_index].get_data_list().shape[0]
            cluster_dsim = self.dsim_search_list[cluster_1_index, cluster_2_index]

            new_dsim_list = np.append(new_dsim_list, ((cluster_1_num) / (cluster_1_num + cluster_2_num)) * dsim_1 + ((cluster_2_num) / (cluster_1_num + cluster_2_num)) * dsim_2 - ((cluster_1_num * cluster_2_num) / ((cluster_1_num + cluster_2_num) ** 2)) * cluster_dsim)
    ...
    def updata(self):
        ...
        new_dsim_list = self.cal_dsim(cluster_1_index = min_dsim_search_list_index[0][0], cluster_2_index = min_dsim_search_list_index[1][0]) #追加する非類似度行列を取得
